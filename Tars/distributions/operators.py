...
class MultiplyDistributionModel(nn.Module):
    ...
    def __init__(self, A, B):
        ...
        If "inherited variables" are exist (which means p(e|c)p(c|a,b)), then A is a child and B is a parent.
        Else (which means p(c|a,b)p(e|c) or p(c|a)p(b|a)), then A is a parent and B is a child.
        """
        ...
        self.prob_factorized_text = self.children_var.prob_factorized_text + self.parents_var.prob_text

        if len(self.cond_var) == 0:
            self.prob_text = "p(" + ','.join(self.var) + ")"            
        else:
            self.prob_text = "p(" + ','.join(self.var) + "|" + ','.join(self.cond_var) + ")"            
    ...
    def sample(self, x=None, batch_size=1, return_all=True, *args, **kwargs):
        ...
        if x is None:
            ...
            if len(self.parents_var.cond_var) > 0:
                parents_input = get_dict_values(x, self.parents_var.cond_var, return_dict=True)
                parents_output = self.parents_var.sample(parents_input, return_all=False)
            else:
                parents_output = self.parents_var.sample(batch_size=batch_size, return_all=False)
            
        # sample from the child distribution
        children_input_inh = get_dict_values(parents_output, self.inh_var, return_dict=True)
        if x is None:
            children_input = children_input_inh
        else:
            children_cond_exc_inh = list(set(self.children_var.cond_var)-set(self.inh_var))
            children_input = get_dict_values(x, children_cond_exc_inh, return_dict=True)
            children_input.update(children_input_inh)
            
        children_output = self.children_var.sample(children_input, return_all=False)
    ...
    def log_likelihood(self, x):
        ...
        parents_x = get_dict_values(x, self.parents_var.cond_var + self.parents_var.var, return_dict=True)
        children_x = get_dict_values(x, self.children_var.cond_var + self.children_var.var, return_dict=True)
        
        return self.parents_var.log_likelihood(parents_x) + self.children_var.log_likelihood(children_x)
